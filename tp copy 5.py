...
def convert_usd_to_inr(amount_usd):
    api_key = '30f463fddaa7d91ba3b74efde98e2aca'
...
def portfolio_worth():
    ...
    for ticker in portfolio.keys():
        ...
        sum_inr = convert_usd_to_inr(sum)
        if sum_inr is not None:
            st.write(f"Your portfolio is worth {sum:.2f} USD or approximately {sum_inr:.2f} INR.")
...
def portfolio_gains():
    ...
    try:
        ...
        relative_gains_inr = convert_usd_to_inr(relative_gains_usd, api_key)
        relative_gains_percent = ((sum_now - sum_then) / sum_then) * 100

        

        st.write(f"Relative Gains (USD): {relative_gains_usd} USD")
        st.write(f"Relative Gains (INR): {relative_gains_inr} INR")
